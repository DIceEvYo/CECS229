...
def sig_change(oldmat, newmat):
	resmat = oldmat - newmat
	for r in range (len(resmat)):
		for c in range(len(resmat[0])):
			if ((resmat[c][r] >= 0.0001) or (resmat[c][r] <= -0.0001)):
				return True
	return False
	#pass
...
def long_run_dist(probs):
	...
	while(sigChange):
		probsprev = probs
		probs = probs.dot(temp)
		#####################################################
		resmat = probs - probsprev
		tot = 0
		for r in range(len(resmat)):
			for c in range(len(resmat[0])):
				if ((resmat[c][r] >= 0.0001) or (resmat[c][r] <= -0.0001)):
					...
					tot += 1
		if(len(resmat) * len(resmat[0]) == tot):
			sigChange = False
		#print(len(resmat) * len(resmat[0]), " ",  tot)
	# print(i , " " , mat)
	return probs
	#pass
